Hello World!! 👋 👋 


# 👩‍💻 Fran Santos

**Analista de Sistemas | Desenvolvedora Web**


💡 Transformo desafios em tecnologia

🚀 Inovação que simplifica

<br/>

## 🧠 Sobre mim

Sou desenvolvedora com foco em soluções tecnológicas eficientes e orientadas a dados.
Tenho interesse em áreas modernas como Inteligência Artificial, Cloud Computing e Cibersegurança.

* 🎓 Formação em Análise e Desenvolvimento de Sistemas
* 💻 Python, JavaScript, SQL
* ☁️ Cloud (deploy de aplicações)
* 🔐 Fundamentos de Cibersegurança
* 📊 Power BI e análise de dados
* 🌐 Inglês avançado | Espanhol intermediário

---

## 🚀 Projetos em destaque

🔹 **Sistema de Chamados (Back-end)**
API para gestão de atendimentos com autenticação
🛠 Python | Flask | SQL
🔗 https://github.com/seuusuario/projeto1

🔹 **Dashboard Inteligente (IA / Dados)**
Análise e previsão de dados com visualização interativa
🛠 Python | Power BI
🔗 https://github.com/seuusuario/projeto2

🔹 **Deploy de Aplicação na Cloud**
Aplicação web publicada com integração contínua
🛠 AWS / Render / Railway
🔗 https://github.com/seuusuario/projeto3

🔹 **Scanner de Portas (Cibersegurança)**
Ferramenta para análise de portas abertas e vulnerabilidades
🛠 Python | Redes
🔗 https://github.com/seuusuario/projeto4

---

## 📂 Projetos por categoria

### 🌐 Front-end

* Landing Page Responsiva
* Clone de Interface

### ⚙️ Back-end

* API REST com autenticação
* Sistema CRUD

### 📊 IA / Dados

* Modelo de previsão com Python
* Dashboard interativo

### ☁️ Cloud

* Deploy de aplicações
* Integração contínua

### 🔐 Cibersegurança

* Scanner de portas
* Análise de vulnerabilidades

---

## 🛠️ Tecnologias

![Python](https://img.shields.io/badge/Python-000?logo=python)
![JavaScript](https://img.shields.io/badge/JavaScript-000?logo=javascript)
![SQL](https://img.shields.io/badge/SQL-000?logo=mysql)
![Power BI](https://img.shields.io/badge/PowerBI-000?logo=powerbi)
![AWS](https://img.shields.io/badge/AWS-000?logo=amazonaws)

![HTML](https://img.shields.io/badge/HTML-E34F26?logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?logo=css3&logoColor=white)
![Kali Linux](https://img.shields.io/badge/Kali_Linux-268BEE?logo=kalilinux&logoColor=white)
![IA](https://img.shields.io/badge/Intelig%C3%AAncia_Artificial-000000?logo=openai&logoColor=white)

---

## 🌐 Portfólio completo

🔗 https://seusite.com

---

## 📫 Contato

📧 [dev.fssantos@protonmail.com](mailto:dev.fssantos@protonmail.com)

💼 LinkedIn: https://linkedin.com/in/dev.fssantos

---



<!--  
**FS-MyCodePath/FS-MyCodePath** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile. />


Here are some ideas to get you started:  

- 🔭 I’m currently working on ...  
- 🌱 I’m currently learning ...  
- 👯 I’m looking to collaborate on ...  
- 🤔 I’m looking for help with ...  
- 💬 Ask me about ...  
- 📫 How to reach me: ...  
- 😄 Pronouns: ...  
- ⚡ Fun fact: ...  
-->
